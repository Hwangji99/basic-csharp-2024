namespace ex19_asyncs
{
    public partial class FrmMain : Form
    {
        ...
        long CopySync(string srcPath,  string destPath)
        {
            ...
            // 파일은 Open()하면 반드시 Close()해야 함 using을 쓰면 Close()를 C#이 알아서 해줌
            // 파일입출력
            ...
            {   // 원래 존재하는 파일을 여니까 FileMode.Open
                ...
                {   // 존재하지 않는 파일을 만드니까 FileMode.Create
                    // 1MByte 버퍼를 생성
                    byte[] buffer = new byte[1024 * 1024]; // 1024(byte) = 1kbyte, 1024 * 1024 = 1Mbyte // 테스트 시 10으로 변경
